...
export function decodeText(bytes: Uint8Array): { text: string; encoding: TextEncoding } {
  ...
  if (!utf8Text.includes("�") && input.equals(roundTrip)) return { text: stripBom(utf8Text), encoding: "UTF-8" };
  const text = iconv.decode(input, "shift_jis");
  if (text.includes("�")) throw new Error("UTF-8 / Shift_JISとして解釈できません");
...
export class ShiftJisEncodeError extends Error {
  constructor(public readonly characters: UnencodableCharacter[]) { super("Shift_JISへ変換できない文字があります"); }
}

export interface UnencodableCharacter { character: string; index: number }

export function findUnencodableCharacters(source: string): UnencodableCharacter[] {
...
const GAIJI: Record<string, string> = { "髙": "［＃「高」の異体字］", "﨑": "［＃「崎」の異体字］", "𠮷": "［＃「吉」の異体字］" };
export function gaijiSuggestion(character: string): string | undefined { return GAIJI[character]; }
