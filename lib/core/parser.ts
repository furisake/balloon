...
export function parse(source: string): ParseResult {
  ...
  const document: DocumentNode = { type: "document", children: blocks, sourceRange: { from: 0, to: source.length } };
  return { document, diagnostics };
}

export function parseBlock(source: string, offset: number, diagnostics: Diagnostic[]): BlockNode {
...
function parseInline(tokens: Token[], diagnostics: Diagnostic[]): InlineNode[] {
  ...
  for (const token of tokens) {
    if (token.type === "text") {
      nodes.push({ type: "text", value: token.value, sourceRange: token.sourceRange });
      continue;
    }
    if (token.type === "ruby") {
      const previous = nodes.at(-1);
      if (!previous || previous.type !== "text") {
        diagnostics.push({ from: token.sourceRange.from, to: token.sourceRange.to, severity: "error", message: "ルビの親文字がありません" });
        nodes.push({ type: "text", value: token.value, sourceRange: token.sourceRange });
        continue;
      }
      ...
      if (!base) {
        diagnostics.push({ from: token.sourceRange.from, to: token.sourceRange.to, severity: "error", message: "ルビの親文字を判定できません" });
        nodes.push({ type: "text", value: token.value, sourceRange: token.sourceRange });
        continue;
      }
      previous.value = previous.value.slice(0, previous.value.length - base.length).replace(/｜$/, "");
      if (!previous.value) nodes.pop();
      nodes.push({ type: "ruby", base, reading: token.value.slice(1, -1), sourceRange: { from: token.sourceRange.from - base.length, to: token.sourceRange.to } });
      continue;
    }
    ...
    if (value.startsWith("ここから傍点") || value === "傍点") {
      ...
      nodes.push({ type: "gaiji", description: value, sourceRange: token.sourceRange });
    } else {
      const image = value.match(IMAGE);
      if (image) nodes.push({ type: "image", src: image[1], alt: "挿絵", width: toNumber(image[2]), height: toNumber(image[3]), sourceRange: token.sourceRange });
