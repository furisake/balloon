...
export function parseCatalogCsv(csv: string): BookFile[] {
  ...
  if (parsed.errors.length && !parsed.data.length) throw new Error("作品一覧CSVを解析できませんでした");
...
export function normalizeCatalogRow(row: CsvRow): BookFile[] {
  ...
  const author = [clean(row[COLUMNS.authorLast]), clean(row[COLUMNS.authorFirst])].filter(Boolean).join(" ");
  const urls = clean(row[COLUMNS.textUrl]).split(/\s*[|、]\s*/).filter((url) => /^https:\/\/www\.aozora\.gr\.jp\//.test(url));
  if (!workId || !title || !urls.length) return [];
  return urls.map((url, index) => {
    const filename = decodeURIComponent(url.split("/").at(-1) ?? `text-${index + 1}`);
    const zip = filename.toLowerCase().endsWith(".zip");
    const encoding = /utf-?8/i.test(filename) ? "UTF-8" : "Shift_JIS";
    return { id: `${workId}:${index}`, workId, title, author, fileId: String(index), filename, format: zip ? "ZIP / TXT" : "TXT", encoding, url };
  });
}
