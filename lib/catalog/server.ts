...
const loadCatalog = unstable_cache(async (): Promise<BookFile[]> => {
...
}, ["aozora-catalog"], { revalidate: APP_CONFIG.cacheTtlSeconds });

export async function getCatalog(): Promise<BookFile[]> { return loadCatalog(); }

export async function resolveBook(workId: string, fileId: string): Promise<BookFile | undefined> {
  return (await getCatalog()).find((book) => book.workId === workId && book.fileId === fileId);
}

export const fetchBookText = unstable_cache(async (workId: string, fileId: string): Promise<{ book: BookFile; text: string }> => {
  ...
  if (book.url.toLowerCase().endsWith(".zip")) {
    const zip = await JSZip.loadAsync(bytes);
    const entry = Object.values(zip.files).find((file) => !file.dir && file.name.toLowerCase().endsWith(".txt"));
    if (!entry) throw new Error("ZIP内に本文がありません");
    bytes = await entry.async("uint8array");
  }
  return { book, text: decodeText(bytes).text };
}, ["aozora-book"], { revalidate: APP_CONFIG.cacheTtlSeconds });
