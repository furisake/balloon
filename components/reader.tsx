...
export function Reader() {
  ...
  useEffect(() => { if (parsed) requestAnimationFrame(() => jumpTo(restoreOffset)); }, [parsed, restoreOffset]);
  function jumpTo(offset: number) { const elements = containerRef.current?.querySelectorAll<HTMLElement>("[data-source-from]"); const target = [...(elements ?? [])].reverse().find((element) => Number(element.dataset.sourceFrom) <= offset); target?.scrollIntoView({ block: "start", inline: "start" }); }
  function savePosition(offset: number) { window.clearTimeout(saveTimer.current); saveTimer.current = window.setTimeout(() => { void saveReadingPosition(id, offset); }, 300); }
