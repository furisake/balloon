...
describe("parse", () => {
  ...
  test.each([["大", 1], ["中", 2], ["小", 3]] as const)("parses a forward-reference %s heading", (name, level) => {
    const result = parse(`女の曲線［＃「女の曲線」は${name}見出し］`);
    expect(result.diagnostics).toHaveLength(0);
    expect(result.document.children[0]).toMatchObject({ type: "heading", level, children: [{ type: "text", value: "女の曲線" }] });
  });

  test("parses a single-line full-width indent", () => {
    expect(parse("［＃７字下げ］御もとへ").document.children[0]).toMatchObject({ type: "paragraph", indent: 7, children: [{ type: "text", value: "御もとへ" }] });
  });

  test("combines an indent and a forward-reference heading", () => {
    expect(parse("［＃５字下げ］一［＃「一」は中見出し］").document.children[0]).toMatchObject({ type: "heading", level: 2, indent: 5, children: [{ type: "text", value: "一" }] });
  });

  test("reports a mismatched forward-reference heading", () => {
    expect(parse("本文［＃「別の文字」は中見出し］").diagnostics).toContainEqual(expect.objectContaining({ severity: "error", message: expect.stringContaining("一致しません") }));
  });
