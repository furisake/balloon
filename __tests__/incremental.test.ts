...
describe("incremental parser", () => {
  ...
  test("multiple paragraph changes equal a full parse", () => {
    const before = "一\n二\n三\n四"; const after = "一\nA\nB\n四";
    expect(parseIncremental(before, after, parse(before)).document).toEqual(parse(after).document);
  });
  test("no change reuses all blocks", () => { const previous = parse("一\n二"); expect(parseIncremental("一\n二", "一\n二", previous).reusedBlocks).toBe(2); });
});
